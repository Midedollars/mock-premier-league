...
const CreateFixtures = async (req, res, next) => {
  try {
    const { stadium, id, homeTeam, awayTeam, date } = req.body;
    if (!stadium || !id || !homeTeam || !awayTeam || !date)
      return res.status(400).json({
        message: "fill in required fields",
      });

    const CreateFixtures = await Fixture.create({
      stadium,
      teams: id,
      homeTeam,
      awayTeam,
...
const editFixtures = async (req, res, next) => {
  try {
    ...
    const editFixtures = await Fixture.findOneAndUpdate({ _id }, req.body, {
      new: true,
    })
      .find()
      .populate("teams", {
...
const viewFixtures = async (req, res, next) => {
  try {
    const viewFixture = await Fixture.find().populate("teams", {
      teamName: 1,
      _id: 0,
    });
    return res.status(200).json({
...
const removeFixtures = async (req, res, next) => {
  try {
    const { _id } = req.headers;
    const removeFixtures = await Fixture.find({ _id });
    return res.status(200).json({
...
const completedFixtures = async (req, res, next) => {
  try {
    ...
    const allCompletedFixs = await Fixture.find().populate("teams", {
      teamName: 1,
      _id: 0,
    });
...
const pendingFixtures = async (req, res, next) => {
  try {
    const pendingFixts = [];
    const allPendingFixs = await Fixture.find().populate("teams", {
      teamName: 1,
      _id: 0,
    });
