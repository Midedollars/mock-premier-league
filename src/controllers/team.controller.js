...
const registerTeam = async (req, res, next) => {
  try {
    ...
    const registerTeams = await Team.create({
      teamName,
      coachName,
      noOfPlayers,
      stadiumName,
...
const updateTeam = async (req, res, next) => {
  try {
    ...
    const updateTeam = await Team.findOneAndUpdate({ _id }, req.body, {
      new: true,
    });
...
const viewTeamsDetails = async (req, res, next) => {
  try {
    const viewTeams = await Team.find();
    return res.status(200).json({
      message: "success",
      viewTeams,
    });
...
const delTeam = async (req, res, next) => {
  try {
    const { _id } = req.headers;
    const delTeam = await Team.findOneAndDelete({ _id });
    return res.status(200).json({
      message: "success",
    });
  } catch (error) {
